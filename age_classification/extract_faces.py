...
def extract_faces(dsfd_weights_path=None, input_images_path=None, extract_faces=True, save_bounding_boxes=True):
    ...
    for img_path in test_img_folder.iterdir():
        ...
        current_image_bounding_boxes[:,[0, 1, 2, 3]] = current_image_bounding_boxes[:,[1, 2, 3, 0]]
        ...
        for face_idx in range(len(current_image_bounding_boxes)):
            ...
            if save_bounding_boxes:
                current_face_details = [str(img_path), current_face[0], current_face[1], current_face[2], current_face[3]]
    ...
    if save_bounding_boxes:
        ...
        all_face_details = pd.DataFrame(all_bounding_boxes, columns=['image_path', 'y_min', 'x_max', 'y_max', 'x_min'])

        # Export dataframe as CSV for processing by embedding generator
        all_face_details.to_csv('all_face_bounding_boxes.csv', encoding='utf-8', index=False)
...
def main():
    ...
    test_img_folder = "test_images/kriti_dataset/18_and_above"
